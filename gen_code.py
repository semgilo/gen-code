...
DEAULT_DST_PATH = os.path.join(
    FILE_PATH, datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'))

FUNCTION_BEGIN_WORDS = ['gen', 'get', 'set', 'check', 'change', 'fix', 'adjust',
                        'attack', 'treat', 'upgrade', 'update', 'load', 'init', 'draw', 'reload']
FUNCTION_MIDDLE_WORDS = ['Big', 'Small', 'Old', 'New', 'Young', 'Short', 'Long', 'Wid', 'Huge', 'Test', 'Preview', 'Red', 'Green',
                         'Blue', 'Yellow', 'Gray', 'Quick', 'Slow', 'Good', 'Bad', 'Cude', 'Randge', 'Circle', 'Rect', 'Round', 'Lucky', 'Usefull', 'Select']

COMMON_WORDS = ['Entity', 'Fruit', 'Enemy', 'Hero', 'Object', 'Computer', 'Animal', 'Vegetable', 'Money',
                'Interface', "Listener", 'Code', 'Writeable', 'WithChannelObject', 'WithSkillObject', 'Food', 'Park']
...
def gen_m_file(parent_dir, classname, variables, extend_classname):
    ...
    for var in variables:
        if var[0] == "NSString*":
            init_func_content = init_func_content + \
    ...
    if extend_classname != '':
        info_func_content = info_func_content + \
            '\tNSString* strSuperInfo = [super getInfo];\n'
    ...
    for var in variables:
        format_content = format_content + \
            var[1] + ' = ' + get_type_flag(var[0]) + ","
        if var[0] == 'NSString*':
            format_args = format_args + 'self.%s' % var[1]
        else:
            format_args = format_args + \
                '[self get%s]' % upper_first_char(var[1])
        index = index + 1
        if index < len(variables):
            format_args = format_args + ','
    info_func_content = info_func_content + \
        '\tstrInfo = [NSString stringWithFormat:@\"%s\", %s]; \n' % (
            format_content, format_args)

    if extend_classname != '':
        info_func_content = info_func_content + \
            '\tstrInfo = [NSString stringWithFormat:@\"%@%@\", strInfo, strSuperInfo]; \n'
...
def gen_class(parent_dir, classname, extend_classname='', variables=[]):
    if len(variables) == 0:
        variables = get_elements_by_random(
            VARIABLE_WORDS, random.uniform(5, 10))
...
def gen_manager_m_file(parent_dir, classname, variables, element_name, import_class_names):
    ...
    write_func(file, 'sharedInstance', classname +
               '*', [], instance_func_content, True)
    file.write('\n')
    ...
    for cn in import_class_names:
        ...
        create_func_content = create_func_content + \
            '\t\t%s *item = [[%s alloc] init];\n' % (cn, cn)
        create_func_content = create_func_content + '\t\t[item getInfo];\n'
        create_func_content = create_func_content + '\t\t[item dealloc];\n'
        create_func_content = create_func_content + '\t}\n'

        index = index + 1
    write_func(file, create_func_name, 'void',
               create_func_args, create_func_content)
    file.write('\n')
...
def gen_manager_class(parent_dir, classname, element_name, variables, import_class_names):
    ...
    gen_manager_h_file(parent_dir, classname, variables,
                       element_name, import_class_names)
    gen_manager_m_file(parent_dir, classname, variables,
                       element_name, import_class_names)
...
def gen_codes(dst_dir, fold_num=5, class_num_per_dir=10, delt_num=3):
    ...
    for fold_name in fold_name_arr:
        ...
        # base class
        base_variables = get_elements_by_random(
            VARIABLE_WORDS, random.uniform(5, 10))
        ...
        for y in range(0, class_num):
            classname = gen_class_name(
                _pre_name, sub_name_arr[y], element_name)
            import_class_names.append(classname)
            variables = get_elements_by_random(
                VARIABLE_WORDS, random.uniform(3, 10))
            for index in range(0, len(variables)):
                var = variables[index]
                variables[index] = (
                    var[0], element_name.lower() + upper_first_char(var[1]))
        ...
        gen_manager_class(dir_path, manager_classname,
                          element_name, base_variables, import_class_names)
    ...
    for x in range(1, 10):
        readme_file.write(
            '-------------------- code %d ----------------------\n' % x)
        elements1 = get_elements_by_random(
            fold_name_arr, random.uniform(1, fold_num))
...
class GenCode(object):
    ...
    def generate(self, output=DEAULT_DST_PATH):
        ...
        gen_codes(output, random.uniform(2, 4), 10, 3)
